...
export class DMChannel {
    ...
    constructor(client: ClientUser, peer: PeerUser, db: IGunInstance, onMessageUpdate: (msg: Message) => void) {
        this.client = client
        this.peer = peer
        this._db = db

        db.get(this.__createChannelQuery()).map().once((d) => {
    ...
    __createChannelQuery() {
        return formatDataStores(`${this.client._sea.epub}-C-${this.peer.epub}`, "chat")
    }
    ...
    async send(content: string) {
        ...
        return new Promise((resolve, reject) => {
            const { clear } = Util.createGunTimeoutRejection("USER FETCH HAS TIMED OUT", reject)
