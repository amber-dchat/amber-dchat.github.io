...
export type SetUserStateHook = React.Dispatch<React.SetStateAction<ClientUser | undefined>>
...
export class AccountManager {
    ...
    constructor(setUserInfo: SetUserStateHook, user: GunUserInstance, db: IGunInstance) {
    ...
    create(username: string, password: string) {
        return new Promise<undefined>(async (resolve, reject) => {
            const doesExists = await new Promise<boolean>((re) => this._db.get(`~@${username}`).once(d => re(!!d)))

            if(doesExists) reject("Unable to create a user account. User with this alias already exists.")
