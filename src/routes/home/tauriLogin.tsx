  ...
  useEffect(() => {
    start(async () => {
      ...
      if (username && password) {
        await account.login(username, password).then(() => delay(1000)).catch(() => { });
      }
    });
  }, [account]);

  return <div className="w-full h-full flex flex-col text-center items-center justify-center select-none">
    {
      processing ?
        <>
        ...
        <Card className="login_width">
          <CardHeader className="flex flex-col">
            <div className="flex w-full py-1 outline-1 outline-dashed rounded-md space-x-1 justify-center mb-2">
              <img src="/favicon.png" className="rounded-full w-7 h-7 mt-auto" />
              <h1 className="text-lg">Amber DChat</h1>
            </div>
            <Tabs onValueChange={setVal} value={val} className="w-full grid gap-2 my-4">
